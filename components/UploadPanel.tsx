...
export function UploadPanel({
  ...
  return (
    <Card
      className={`surface-shell sidebar-scroll xl:sticky xl:top-4 ${
    ...
    >
      ...
      <CardContent className="space-y-4">
        ...
        >
          ...
          <p className="mt-3 text-sm font-medium text-zinc-950">Upload source docs</p>
          ...
          >
            {isAtSessionLimit ? "Session limit reached" : isUploading ? "Ingesting…" : "Upload docs"}
          </Button>
          <input
            ref={inputRef}
            id={inputId}
          ...
          <>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">Most recent processing run</p>
              </div>
              <div className="space-y-2">
                {uploadOutcomes.map((outcome) => (
                  <div key={`${outcome.filename}-${outcome.status}`} className="surface-muted p-3 text-sm">
                    <div className="flex items-start justify-between gap-3">
                      <p className="text-zinc-700">{outcome.filename}</p>
