...
1. Extract shared claim-generation branching from `src/server/investigation/investigate.ts` and `src/server/investigation/graph/nodes/generate-claims.ts` into `lib/claim-generation.ts`.
