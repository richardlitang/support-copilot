...
describe("chunkParsedDocument", () => {
  ...
  it("keeps troubleshooting table rows with their corrective actions", () => {
    ...
    const oilUsageChunks = chunks.filter((chunk) => chunk.content.includes("Compressor uses too much oil"));
    ...
    expect(chunks.some((chunk) => chunk.content.includes("Wrong oil being used") && chunk.content.includes("Drain and replace oil"))).toBe(
      true
    );
  });
});
