...
export function getReviewAction(result: InvestigationResult): ReviewAction | null {
  ...
  if (result.reviewDecision.action === "add_context") {
    return {
      ...
      description:
        "The current run needs plan, feature, recent-error, or support-note context before it can produce a supported answer.",
  ...
  if (result.reviewDecision.action === "add_docs") {
    return {
      ...
      description:
        "The retrieved documentation was not enough to support the answer. Upload the missing runbook, FAQ, or policy doc and rerun.",
  ...
  if (result.reviewDecision.action === "inspect_conflict") {
    return {
      ...
      description:
        "The documentation and tool state do not explain the reported issue. Inspect the evidence, add the missing context, then rerun.",
      primaryActionLabel: "Add context and retry",
    };
  }
  ...
  return {
    ...
    description:
      "The system could not verify enough cited claims for a ready answer. Review the evidence before replying to the customer.",
